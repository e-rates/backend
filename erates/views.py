...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        description="Retrieve users with overdue payments that have surpassed their deadline. Admin/Auditor only.",
        tags=['Payments'],
        parameters=[
            OpenApiParameter('min_days_overdue', OpenApiTypes.INT, description='Minimum days overdue to filter'),
        ...
        responses={200: OpenApiResponse(description="List of defaulters with summary statistics")},
    )
    @action(detail=False, methods=['get'], permission_classes=[IsAdminOrAuditor])
    def defaulters(self, request):
        """Get list of users with overdue payments"""
        from .serializers import DefaultersSummarySerializer
        ...
        ).select_related('user', 'account').order_by('deadline')
        ...
        if min_days:
            try:
                ...
                overdue_payments = overdue_payments.filter(deadline__lte=cutoff_date)
            except ValueError:
                pass
        ...
        for payment in overdue_payments:
            ...
            # Track by currency
            if payment.currency not in currency_breakdown:
                currency_breakdown[payment.currency] = {
                    'count': 0,
                    'total_amount': Decimal('0.00')
                }
            currency_breakdown[payment.currency]['count'] += 1
            currency_breakdown[payment.currency]['total_amount'] += payment.amount
        ...
        currency_breakdown_serialized = {
            k: {'count': v['count'], 'total_amount': str(v['total_amount'])}
            for k, v in currency_breakdown.items()
        }
        
        # Calculate summary
        avg_days_overdue = total_days_overdue / len(defaulters) if defaulters else 0
        
        summary = {
            'total_defaulters': len(defaulters),
            'total_overdue_amount': total_overdue,
            'average_days_overdue': round(avg_days_overdue, 2),
            'defaulters_by_currency': currency_breakdown_serialized,
            'defaulters': defaulters,
        }
        
        serializer = DefaultersSummarySerializer(summary)
        return Response(serializer.data)
